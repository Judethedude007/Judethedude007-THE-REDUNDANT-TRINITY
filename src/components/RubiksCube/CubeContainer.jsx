...
import { cubeWidth, facePosition } from './constants';
import {
    calcPosition,
    calculateResultantAngle,
    getCubePositionDifference,
    getTouchPositions
} from './utilities';
...
class CubeContainer extends Component {
    constructor(props) {
        ...
        // Create complete, solvable Rubik's cube positions
        ...
        this.state = {
            ...
            faceRotationAngle: 0,
            isSolvable: props.isSolvable !== undefined ? props.isSolvable : true
        };
        ...
        this.swapTwoPieces = this.swapTwoPieces.bind(this);
    }
    ...
    componentDidMount() {
        ...
        // Expose swapTwoPieces method to parent via ref
        if (this.props.onRef) {
    ...
    rotateCube(xAxisMove, yAxisMove, cubePosition, touchedFace, cubeOrientation) {
        // Avoid face rotation while auto move
        ...
        if (touchedFace) {
            ...
            // Check for face rotation vector defined
            if (this.state.faceRotationAngle) {
                ...
                if (diff > getCubePositionDifference(movedPosition, cubePosition, xAxisMove, yAxisMove)) {
                    diff = getCubePositionDifference(movedPosition, cubePosition, xAxisMove, yAxisMove);
                    reverseAngle = false;
                }
    ...
    swapTwoPieces() {
        if (this.state.isSolvable) {
            const newPositions = [...this.state.positions];
            // Swap two corner pieces (indices 5 and 7) to make it unsolvable
            [newPositions[5], newPositions[7]] = [newPositions[7], newPositions[5]];
            this.setState({ 
                positions: newPositions, 
                isSolvable: false 
            });
        } else {
            // Reset to solvable state
